...
<div class="container">
    ...
    <div class="w-full px-4">
        ...
        <form id="signup_form" method="POST" action="{{ route('register') }}">
            ...
            class="border-stroke  bg-transparent w-full rounded-md border py-3 px-5 text-base text-body-color  placeholder:text-dark-6 outline-none transition focus:border-primary  focus-visible:shadow-none  @error('name') is-invalid @enderror" />
            </div>
            
            <!-- email -->
            <div class="mb-[22px]">
            <input id="email" type="email" placeholder="name@example.com" name="email" value="{{ old('email') }}" required autocomplete="email"
        ...
        <p class="mb-4 text-base text-body-secondary">
            ...
            <a href="{{route('policy')}}" class="text-primary hover:underline">
            Policy
            </a>
            and
